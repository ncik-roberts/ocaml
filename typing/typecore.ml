...
let type_approx_fun label ret_ty =
  let ty = if is_optional label then type_option (newvar ()) else newvar () in
...
let rec type_approx env sexp =
  ...
  | Pexp_fun (p, _, _, e) -> type_approx_fun p (type_approx env e)
  ...
  | [] ->
    let body_ty =
      ...
      | Pfunction_cases ({pc_rhs = e} :: _, _, _) ->
          type_approx_fun Nolabel (type_approx env e)
      | Pfunction_cases ([], _, _) ->
          newvar ()
    in
...
let check_partial ?lev a b c d cases =
  check_partial ?lev a b c d (List.map Parmatch.typed_case cases)
