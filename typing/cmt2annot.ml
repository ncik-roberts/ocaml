...
let bind_function_param fp =
  let loc = fp.fp_loc in
  match fp.fp_kind with
  | Param_pat pat -> bind_variables loc pat
  | Param_optional_default (pat, _expr) -> bind_variables loc pat
...
let rec iterator ~scope rebuild_env =
  ...
  and expr sub exp =
    ...
    | Texp_try (_, f) ->
        bind_cases f
    | Texp_function { params } ->
        List.iter bind_function_param params
